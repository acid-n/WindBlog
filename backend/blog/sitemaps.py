...
from django.urls import reverse # Для get_absolute_url модели, если он нужен
...
class PostSitemap(Sitemap):
    ...
    try:
        ...
        default_changefreq = settings_instance.default_sitemap_changefreq if settings_instance else "weekly"
        default_priority = settings_instance.default_sitemap_priority if settings_instance else 0.8 # Повысим приоритет для постов
    except Exception:
    ...
    def items(self):
        ...
        return Post.objects.filter(is_published=True, sitemap_include=True).order_by('-first_published_at')
    ...
    # Переопределяем методы для использования значений из модели или дефолтных
    def priority(self, obj):
        return obj.sitemap_priority if obj.sitemap_priority is not None else self.default_priority

    def changefreq(self, obj):
        return obj.sitemap_changefreq if obj.sitemap_changefreq else self.default_changefreq 