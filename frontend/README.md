...
В каталоге `frontend` расположен файл `.env.local.example`. Скопируйте его в `.env.local` и при необходимости измените значения. При запуске через Docker-compose этот файл будет использован для передачи `NEXT_PUBLIC_API_BASE` внутрь контейнера.
