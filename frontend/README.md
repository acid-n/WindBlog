...
- **Шрифты:** Lora, Coustard и Inter подключены локально через `@font-face`. Шрифты Inter встроены прямо в `globals.css` в формате `data:`-URI, поэтому внешние запросы к Google Fonts не выполняются.
> Для подключения новых шрифтов поместите файлы в удобную папку проекта и добавьте правила `@font-face` в `globals.css`.
