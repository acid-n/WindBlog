...
# Скрипт для универсальной сборки Next.js для ARM64 с поддержкой SSR
# Работает как на x86_64, так и на ARM64, даже с проблемными нативными модулями

echo "🚀 Запуск универсальной сборки Next.js для ARM64 с полной поддержкой SSR"
...
# 2. Создаем оптимизированную конфигурацию Next.js для SSR
echo "📝 Создаем оптимизированную конфигурацию Next.js для совместимости с ARM64 и SSR..."
...
# 5. Запуск сборки Next.js с полной поддержкой SSR
echo "🏗️ Запуск сборки Next.js..."
npm run build

echo "✅ Сборка успешно завершена! SSR-версия готова к запуску."
