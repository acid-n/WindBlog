...
export async function generateMetadata({ params: paramsProp }: { params: { slug: string } }) {
  ...
  try {
    ...
    return {
      ...
      description: post.description || post.short_description || `Пост "${post.title}" в блоге`,
      openGraph: {
        title: post.title,
        description: post.description || post.short_description || `Пост "${post.title}" в блоге`,
    ...
    const errorMessage = e instanceof Error ? e.message : "An unknown error occurred while fetching metadata";
 ...
 */
const PostPage = async ({ params: paramsProp }: { params: { slug: string } }) => {
  ...
  try {
    ...
    const errorMessage = e instanceof Error ? e.message : "An unknown error occurred while fetching post";
  ...
  const shortUrl = post.shortlink ? 
    (post.shortlink.full_url || `${process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3000'}${post.shortlink.url}`) : 
    postUrl;
    ...
    <section className="flex flex-col items-center pt-[15px] w-full">
      <article className="flex flex-col items-center max-w-4xl w-full mx-auto">
        ...
        <div className="w-full max-w-[800px] px-4 md:px-0">
          ...
          <div className="flex flex-wrap items-center justify-center gap-x-2 text-lg text-[#b3b3b3] text-center mb-8" style={{ lineHeight: 1.5 }}>
            <span>пост в</span> 
          ...
          <div className="prose prose-lg w-full text-[#444]" style={{ fontFamily: "'Lora', serif", lineHeight: 1.8, letterSpacing: '0.02em' }}>
